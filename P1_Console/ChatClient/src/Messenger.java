...
public class Messenger implements Runnable{
	...
	Messenger(Console console, PostBoard postBoard, Socket clientSocket, PrintWriter out, BufferedReader in){
			...
			while( (sInstruction = in.readLine()) != null ){
				...
				if( sCmd.equals("/msg") ) msg( sTokens );
					...
					if( bKicked ) {
						send( "/leave" );
						stop();
						console.setOffline(); //要將console中的m_bOnline設為false
						break;
					}
				}
			}
	...
	private void post( String sTokens[] ){
		...
		postBoard.add( post );
	    System.out.println( sUserName + " posted message '" + iPostId 
	...
	private void remove( String sTokens[] ){
		...
		System.out.println( sUserName + " remove message '" + iPostId + "': " + post.m_sContent );
