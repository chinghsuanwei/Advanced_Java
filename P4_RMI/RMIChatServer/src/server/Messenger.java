...
public class Messenger extends Thread
{
	...
	private String[] parse( String sInstruction ){
		...
		//先去掉前面的空白
		int found = sInstruction.trim().indexOf(" ");
		if( found > 0 ) sCmd = sInstruction.substring(0, found);
		...
		else if( sCmd.equals("/post") || sCmd.equals("/tell") || sCmd.equals("/change") ) limit = 3;
		else limit = -1;
	...
	private void post( String sTokens[] ){
	    ...
	    * "/post String msg": Client sends this message to server. In server, assign a series id "msgid" to the message, 
		...
		try{
			...
			if( !postBoard.isLegalContentType(sContentType) ) throw new NoSuchPostType();
			if( postBoard.isWidgetType( sContentType ) && 
				!postBoard.isWidgetFormatRight( sContent ) ) throw new WidgetFormatException();
			Post post = new Post( getUserId(), sContentType, sContent );
			postBoard.add(post);
		...
		} catch ( WidgetFormatException e ){
			
			send( "/msg Error: WidgetFormatException." );
		}
	}
	...
	private void remove( String sTokens[] ){
		...
		try {
			...
			postBoard.remove( I.getUserId(), iPostId ); //NoSuchMessageIdException
			...
			send( "/msg Error: Permission denied." );
		}
	...
	private void change( String sTokens[] ){
		...
		try{
			...
			//NoSuchMessageIdException
			if( postBoard.findOwnerId( iWidgetId ) != getUserId() ) send( "/msg Error: Permission denied." );
			else{
				postBoard.changeWidgetProperty( iWidgetId, sProperty );
				broadcast( "/change " + iWidgetId + " " + sProperty );
			}
		} catch ( NoSuchMessageIdException e ){
			
			send( "/msg Error: No such msg id." );
		} catch ( NotWidgetException e ){
			
			send( "/msg Error: NotWidgetException." );
		} catch ( ArrayIndexOutOfBoundsException e ){
			
			send( "/msg Error: ArrayIndexOutOfBoundsException.");
		}		 
	}
	...
	private void move( String sTokens[] ){
		...
		try{
			...
			if( x < 0 || y < 0 ){
				send( "/msg Error: Coordinate must be positive." );
				return;
			}
			...
			send( "/msg Error: NumberFormatException." );			
		} catch ( ArrayIndexOutOfBoundsException e ){
			
			send( "/msg Error: ArrayIndexOutOfBoundsException1.");
		}
	}
